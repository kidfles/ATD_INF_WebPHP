            ...
            <div class="bg-slate-800/40 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 sm:p-8">
                ...
                <form action="{{ route('dashboard.advertisements.update', $advertisement) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="date" name="expires_at" value="{{ old('expires_at', $advertisement->expires_at ? $advertisement->expires_at->format('Y-m-d') : '') }}" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-violet-500/50 focus:ring-violet-500/20 focus:outline-none transition" min="{{ date('Y-m-d', strtotime('+1 day')) }}">
                    </div>
