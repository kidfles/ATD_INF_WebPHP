            ...
            <div class="bg-white rounded-[2rem] shadow-soft border border-slate-100 p-6 sm:p-8">
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                    ...
                    <div class="bg-slate-50 border border-slate-100 p-5 rounded-2xl h-full relative">
                        ...
                        {{-- Status Logic --}}
                        @if(auth()->user()->companyProfile->contract_status === \App\Enums\ContractStatus::Approved)
                        ...
                        @elseif(auth()->user()->companyProfile->contract_status === \App\Enums\ContractStatus::Rejected)
                            <div class="flex items-center bg-red-50 border border-red-200 text-red-700 p-3 rounded-xl mb-4">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                                <div>
                                    <span class="font-bold block">{{ __('Rejected') }}</span>
                                    <span class="text-xs text-red-600">{{ __('Please upload a correct version.') }}</span>
                                </div>
                            </div>
                        @else
            ...
            <form method="post" action="{{ route('dashboard.company.settings.update') }}" class="space-y-6">
                ...
                <div class="bg-white rounded-[2rem] shadow-soft border border-slate-100 p-6 sm:p-8">
                    ...
                    <div class="mt-6 space-y-6 max-w-xl">
                        ...
                        @if($company->contract_status === \App\Enums\ContractStatus::Approved)
